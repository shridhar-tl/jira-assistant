...
const UserProjectWiseSummary = function ({ groups, flatWorklogs, exportSheetName, convertSecs, costView }) {
    const [state, setState] = useState(false);
    useEffect(() => {
        setState(getGroupedData(groups, flatWorklogs));
    }, [groups, flatWorklogs]);
    ...
    const { projects, groupedData, grandTotal } = state;
    ...
    return (
        <ScrollableTable className="table-bordered" dataset={groupedData} exportSheetName={exportSheetName}>
            ...
            <TBody>
                {(group, g) => (<UserGroup key={g} group={group} projects={projects} costView={costView} convertSecs={convertSecs} hideGroup={hideGroup} />)}
            </TBody>
...
const Footer = memo(function ({ grandTotal, projects, costView, convertSecs }) {
    return (<tfoot>
        <tr className="strong data-center">
            ...
            {costView && projects.map((project, pi) => (<Fragment key={pi}>
                {project.issueTypes.map((it, iti) => <td key={iti} exportType="float"
                    title={convertSecs(grandTotal[project.key][it])}>{grandTotal[project.key][`${it}_Cost`]}</td>)}
...
function getGroupedData(groups, flatData) {
    const projects = [];
    const projectsData = {};
