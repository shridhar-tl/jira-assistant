...
import type { CustomHeaders } from './ajax-request-service';
...
export default class AjaxService {
    ...
    constructor($session: SessionService, $message: MessageService, $request: AjaxRequestService, $jAuth: any) {
    ...
    prepareUrl(url: string, params?: any[] | Record<string, any>): string {
    ...
    handler<T = any>(req: Promise<T>, quiet?: boolean): Promise<T> {
        return req.then(null, (e: any) => {
            if (!quiet && e.status === 0) {
    ...
    request<T = any>(method: string, url: string, params?: any, headers?: CustomHeaders, quiet?: boolean): Promise<T> {
    ...
    async execute<T = any>(method: string, url: string, params?: any, customHeaders?: CustomHeaders): Promise<T> {
        const headers = await this.$jAuth.transformHeaders(this.$session.userId, customHeaders);
        return this.$request.execute(method, url, params, headers);
    }
