...
type ValidatorFn = (json: any, result: any, leftPart?: any) => any;

export function parseWhereClause(filterText: string) {
...
export function validateFilterText(filterText: string, scope: any): FilterValidationResult {
    try {
...
function validateFilterJson(json: any, scope: any): FilterValidationResult {
    ...
    try {
        ...
        return { isValid: false, errorMessage: err.message, json };
    }
}

function validateExpression(json: any[], result: any) {
    json.forEach(obj => {
        if (!comparers[obj.type as keyof typeof comparers]) {
...
function validateObject(json: any, result: any, leftPart?: any): any {
    const { type, operator } = json;
    const expressionFunc = expressionType[type as keyof typeof expressionType];
...
const derivedTypes: Record<string, string> = { date: 'datetime' };
function normalizeReturnType(returnType: any): any {
    ...
    if (Array.isArray(type)) {
        type = type.map((t: string) => derivedTypes[t] || t).filter((v: string, i: number, a: string[]) => a.indexOf(v) === i);
        if (type.length < 2) {
            type = type[0];
        }
...
const comparers: Record<string, ValidatorFn> = {
...
const expressionType: Record<string, ValidatorFn> = {
    ...comparers,
...
function validate_LeftAndRightExpression(andExpr: any, result: any) {
...
function validate_ComparisonBooleanPrimary(eqExpr: any, result: any) {
...
function validate_BetweenPredicate(eqExpr: any, result: any) {
    const {
        left: leftField,
        right: { left, right },
    } = eqExpr;
...
function validate_LikePredicate(likeExpr: any, result: any) {
    ...
    if (value.replace(/[%.*]/g, '').length < 3) {
        // Length includes enclosing double quotes
...
function validate_InExpressionListPredicate(expr: any, result: any) {
...
function validate_ExpressionList(expr: any, result: any, leftPart: any) {
    const { value } = expr;
...
function validate_Identifier(obj: any, result: any) {
    const { value } = obj;
...
function validate_FunctionCall(obj: any, result: any, leftPart: any) {
...
function validateParametersFunctionArgs(funcName: string, params: any[], result: any, leftPart: any) {
    if (funcName === paramsFunctionName) {
        const nameParam = params[0];
        const paramType = nameParam.type.toLowerCase();

        if (paramType !== 'string') {
            // As string is compatible with any type, above check will not handle this case
        ...
        if (paramNameWithoutQuotes.length < 5) {
            // Param name would be enclosed with "". Hence considered 3 char param name
            throw new Error('Parameter name is too short');
        }

        if (paramNameWithoutQuotes.length > 27) {
            // Param name would be enclosed with "". Hence considered 25 char param name
...
function validate_direct_values(obj: any) {
    const { type } = obj;

    return { type: type.toLowerCase() };
}

function compareTypes(type1: any, type2: any): boolean {
    if (!type1 || !type2) {
        return false;
    }
    ...
    return type1.some((t: string) => type2.includes(t));
}

function convertToComparableType(type: any): any {
