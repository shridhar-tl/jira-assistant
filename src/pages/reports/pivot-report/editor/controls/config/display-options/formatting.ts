...
export const valuePropsMap = formatOptions.reduce((map: Record<string, any>, cur) => {
    map[cur.value] = cur.props;
    return map;
}, {});

const grouped = formatOptions.reduce((acc: Record<string, any[]>, item) => {
    if (!acc[item.type]) {
        acc[item.type] = [];
    }
...
export const typeFormatMap: Record<string, any[]> = grouped;
